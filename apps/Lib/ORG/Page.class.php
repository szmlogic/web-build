...
class Page extends Think {
     ...
     */
    public function __construct($totalRows,$listRows,$p='',$parameter='') {
        ...
        $this->totalPages = ceil($this->totalRows/$this->listRows);     //总页数
            if (!defined('PAGESTOTAL'))
                define('PAGESTOTAL', $this->totalPages);
    ...
    public function  show(){
        ...
        $output .= '<a class="previous_page" href="'.$this->pageurl($urlrule, $pre_page,$this->parameter).'">'.L('previous').'</a>';
        ...
        $output .='<a class="next_page" href="'.$this->pageurl($urlrule,$next_page,$this->parameter).'">'.L('next')."</a>";
    ...
    public function  wap_show()
    {
        ...
        if($this->totalPages <= $show_nums){
            for($i = 1;$i<=$this->totalPages;$i++){
                if($i == $this->nowPage){
                    $output .= '<span class="cur_page">'.$i.'</span>';
                }else{
                    $output .= '<a href="'.$this->pageurl($urlrule,$i,$this->parameter).'">'.$i.'</a>';
                }
            }
