...
class FormAction extends PublicAction {
    ...
    function delete()
    {
        ...
        if($m){
            ...
            savecache('Form');
            @unlink(RUNTIME_PATH.'Data/'.$id.'_FormField.php');
    ...
    function content()
    {
        ...
        if ($count > 0) {
            ...
            //分页查询数据
            $voList = $form_db->where($map)->limit($p->firstRow . ',' . $p->listRows)->select ( );
            ...
            $page->urlrule = U($form_db.'/content', $map);
            //分页显示
            $page = $p->show();
