...
function clean_thumb(inputid){
    $('#'+inputid+'_pic').attr('src',PUBLIC+'/images/upload_thumb.png');
    $('#'+inputid).val('');
...
//图片列表
function up_image(iframeWin, topWin,inputid){
...
function upload_files(iframeWin, topWin, id, inputid){
    var data = '';
    var num = iframeWin.$('#myuploadform > div').length;
    if(num){
        data = iframeWin.$('#myuploadform').html();
        $('#'+inputid+'_images').append(data);
    }
}
