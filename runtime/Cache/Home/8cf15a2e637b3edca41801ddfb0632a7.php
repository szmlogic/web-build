...
<body>
 ...
 $count=0; foreach ($Cats as $keyy=>$vy) { if($vy["ismenu"]==1 && intval($r['id'])==$vy["parentid"]) { $count++; } } $n=0; foreach ($Cats as $key=>$rr) { if( $rr['ismenu']==1 && intval($r['id'])==$rr["parentid"]) { ++$n; ?><li>
    ...
    <div class="left_news">
        ...
        <ul class="left_news">
            <?php
 $_result=M("Article")->field("thumb,title,url,createtime")->where("createtime<=1560344101 AND status=1  AND catid in(5,11,52,12,13) AND posid like '%-1-%'")->order("id desc")->limit("5")->select(); if ($_result) { $i=0; $total = count($_result); foreach($_result as $key=>$r) { ++$i; $mod = ($i % 2 ); ?><li>
                    <a href="<?php echo ($r["url"]); ?>" title="<?php echo ($r["title"]); ?>">
