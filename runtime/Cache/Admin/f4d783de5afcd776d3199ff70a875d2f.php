...
<div class="table-toolbar">
    <a title="新增" class="btn btn-primary" href="<?php echo U('Menu/add');?>">新增</a>&nbsp;
</div>
        ...
        <form id="myform" name="myform" action="<?php echo U('menu/listorder');?>" method="post">
            <table class="table table-hover table-bordered">
                ...
                <?php if(is_array($menu_list)): $i = 0; $__LIST__ = $menu_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo ($vo["title_prefix"]); ?><a href="<?php echo U('Menu/index','pid='.$vo['id']);?>"><?php echo ($vo["name"]); ?></a></td>
                        <td><?php if(!empty($vo["icon"])): ?><i class="<?php echo ($vo["icon"]); ?>"></i><?php endif; ?></td>
                        <td><?php echo ($vo["listorder"]); ?></td>
                        <td>
                            <?php if(($vo["status"]) == "1"): ?><a href=""><i class="fa fa-check text-success"></i></a>

                                <?php else: ?>
                                <a href="javascript:;"><i class="fa fa-ban text-danger"></i></a><?php endif; ?>
                        </td>
                        <td>
                            <a class="btn btn-xs btn-primary" href="<?php echo U('menu/add',array( 'parentid' => $vo['id']));?>">添加子菜单</a>
                            <a class="btn btn-xs btn-info" href="<?php echo U('menu/edit',array('id' => $vo['id']));?>">修改</a>
                            <a class="btn btn-xs btn-danger" href="javascript:confirm_delete('<?php echo U('Menu/delete',array( 'id' => $vo['id']));?>')">删除</a>
                        </td>
