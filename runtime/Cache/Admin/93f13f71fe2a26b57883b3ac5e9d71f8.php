...
<div class="container-fluid">
    ...
    <div class="table-toolbar">
        <a class="btn btn-primary" href="<?php echo U('Model/add');?>">添加模型</a>
    </div>

    <form name="myform" action="<?php echo U('Model/listorder');?>" method="post">
        <table class="table table-bordered table-hover table-condensed">
            ...
            <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                    ...
                    <td><?php if($vo[status]==1): ?><a class="btn btn-xs btn-success" href="<?php echo U('Model/status',array(id=>$vo['id'],status=>0));?>">启用</a>
                        <?php else: ?>
                        <a class="btn btn-xs btn-danger" href="<?php echo U('Model/status',array(id=>$vo['id'],status=>1));?>">禁用</a><?php endif; ?></td>
                    <td class="manage w-200">
                        <a class="btn btn-xs btn-primary" href="<?php echo U('Field/index',array(modelid=>$vo['id']));?>">模型字段</a>
                        <a class="btn btn-xs btn-primary" href="<?php echo U('Model/edit',array(id=>$vo['id']));?>">编辑</a>

                        <?php if($vo['issystem']): ?><a class="btn btn-xs btn-default" disabled>删除</a>
                        <?php else: ?>
                            <a class="btn btn-xs btn-primary" href="<?php echo U('Model/delete',array(id=>$vo['id']));?>"  onclick="return confirm('您确定要卸载吗？')">删除</a><?php endif; ?>
                    </td>
