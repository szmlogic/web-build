...
<div class="container-fluid">
    ...
    <div class="table-toolbar">
        <a href="<?php echo U('Slide/index');?>" class="btn btn-sm btn-primary"><i class="fa fa-reply"></i>返回广告列表</a>
        <a href="<?php echo U('Slide/addpic?fid='.$slide[id]);?>" class="btn btn-sm btn-primary">添加图片</a>
        <a class="btn btn-sm btn-primary" onclick="myform.action='<?php echo U('Slide/listorder');?>';$('#myform').submit();">排序</a>
    </div>
    ...
    <form name="myform" id="myform" action="" method="post">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th width="48"><?php echo L('listorder');?></th>
                    <th align="left"><?php echo L(THUMB);?>&nbsp;&nbsp;</th>
                    <th width="140" class="center"><?php echo L('manage');?></th>
                </tr>
            </thead>
            <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                    <td width="40" class='center'><input name='listorders[<?php echo ($vo["id"]); ?>]' type='text' size='3' value='<?php echo ($vo["listorder"]); ?>' class='input-text-c'></td>
                    <td align="left"><img src="<?php echo ($vo['pic']); ?>" onload="if(this.width >= 320){this.width = 320}"></td>
                    <td class="center">
                        <a class="btn btn-xs btn-primary" href="<?php echo U('slide/editpic',array(fid=>$fid,id=>$vo['id']));?>">编辑</a>
                        <a class="btn btn-xs btn-danger" href="javascript:confirm_delete('<?php echo U('slide/deletepic',array(id=>$vo['id']));?>')">删除</a>
                    </td>
